...
1) Рекурсія — це техніка програмування, коли метод або функція викликає саму себе неодноразово,
 доки не досягне базового виразу.

2) Stream: це абстракція, яка представляє послідовність елементів і підтримує такі операції функціонального стилю,
 як фільтрація, відображення, скорочення тощо. Це дозволяє обробляти колекції (масиви, списки тощо)  стисло, за допомогою лямбда-виразів. У Stream немає
 можливості для обробки Exception.
 Optional: це підвид Stream, але з функціями, які дозволяють додавати умови для отримання алементів, а також обробляти випадки, коли елемент не знайдено.
 Що дозволяє створювати та обробляти Exception, та при виконанні тесту бачити, де виникла проблема.
*/
}
